...
func TestDatsetMetrics(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			propsRequested: []string{`available`, `compressratio`, `logicalused`, `logicalreferenced`, `quota`, `refcompressratio`, `referenced`, `refquota`, `refreservation`, `reservation`, `snapshot_count`, `snapshot_limit`, `used`, `usedbychildren`, `usedbydataset`, `usedbyrefreservation`, `usedbysnapshots`, `volsize`, `written`, `creation`, `keystatus`},
...
# HELP zfs_dataset_keystatus Indicates if the encryption key is currently loaded [0: none, 1: available, 2: unavailable]
# TYPE zfs_dataset_keystatus gauge
zfs_dataset_keystatus{name="testpool/test",pool="testpool",type="filesystem"} 1
`,
		},
		{
			name:           `multiple pools`,
